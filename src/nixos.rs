...
pub fn vix_nixos(argument: &String, installion_argument: String) {
    match argument.as_str() {
        ...
        "remove-env" => {
            ...
            if output.status.success() {
                println!(
                    "Succesfully deleted package: {}",
                    installion_argument.clone()
                );
            } else {
                let stderr = String::from_utf8_lossy(&output.stderr);
                eprintln!("Failed to execute command. error: {}", stderr);
        ...
        "--help" => {
            println!("install-env <package name>: installs the specified package");
            println!("remove-env <package name>: deletes specified package");
